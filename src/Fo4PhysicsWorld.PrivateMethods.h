...
// Split from Fo4PhysicsWorld.h: private method declarations.
		...
		BuildResult BuildSystemObjectsLocked(Fo4SkinnedMeshSystem& a_state, const LifecycleEvent& a_event, const PhysicsXmlSummary& a_summary, const DefaultBBP::NameMap& a_meshNameMap, BuildDomain a_domain, bool a_commitToBullet = true, std::span<const RE::NiPointer<RE::NiAVObject>> a_meshSourceRoots = {});
		bool BuildMeshesLocked(Fo4SkinnedMeshSystem& a_state, const PhysicsXmlSummary& a_summary, const LifecycleEvent& a_event, const DefaultBBP::NameMap& a_meshNameMap, std::span<const RE::NiPointer<RE::NiAVObject>> a_meshSourceRoots, std::uint64_t a_buildGroup, BuildDomain a_domain, std::vector<BoneRecord>& a_stagedBodies, std::vector<MeshRecord>& a_stagedMeshes);
