...
// Split from Fo4PhysicsWorld.h: private method declarations.
		...
		std::uint32_t CollectHeadPartGroupsLocked(const Fo4SkinnedMeshSystem& a_state, std::vector<std::uint64_t>& a_buildGroups) const;
		bool HasActiveHairSlotArmorLocked(const Fo4SkinnedMeshSystem& a_state) const;
		...
		BuildResult BuildSystemObjectsLocked(Fo4SkinnedMeshSystem& a_state, const LifecycleEvent& a_event, const PhysicsXmlSummary& a_summary, const DefaultBBP::NameMap& a_meshNameMap, BuildDomain a_domain, bool a_commitToBullet = true);
		bool BuildMeshesLocked(Fo4SkinnedMeshSystem& a_state, const PhysicsXmlSummary& a_summary, const LifecycleEvent& a_event, const DefaultBBP::NameMap& a_meshNameMap, std::uint64_t a_buildGroup, BuildDomain a_domain, std::vector<BoneRecord>& a_stagedBodies, std::vector<MeshRecord>& a_stagedMeshes);
		void BuildConstraintsLocked(Fo4SkinnedMeshSystem& a_state, const PhysicsXmlSummary& a_summary, std::uint64_t a_buildGroup, BuildDomain a_domain, std::span<BoneRecord> a_stagedBodies, std::vector<ConstraintRecord>& a_stagedConstraints);
